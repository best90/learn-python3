...
class ProgressBar(object):
    def __init__(self, title, count=0.0, run_status=None, finish_status=None, total=100.0, unit='', sep='/', chunk_size=1.0):
    ...
    def refresh(self, count=1, status=None):
        ...
        if self.count >= self.total:
            end_str = '\n'
            self.status = status or self.finish_status
...
if __name__ == '__main__':
    ...
    with closing(requests.get(url, stream=True)) as response:
        ...
        if response.status_code == 200:
            ...
            progress = ProgressBar("%s" % filename
			            , total = content_size
			            , unit = "KB"
